...
const App = () => {
  ...
  // const signleMaskModel =
  //   "/assets/data/spatImageWithExtraDataWithSingleMask.onnx";
  // const multiMaskModelOnnx = "/assets/data/spatImageWithExtraData.onnx";

  const signleMaskModel =
    "/assets/interactive_module_quantized_592547_2023_03_19_sam6_long_uncertain.onnx";
  const multiMaskModelOnnx =
    "/assets/interactive_module_quantized_592547_2023_03_20_sam6_long_all_masks_extra_data_with_ious.onnx";

  useEffect(() => {
    const initModel = async () => {
      try {
        // if (process.env.MODEL_DIR === undefined) return;
        const URL: string = signleMaskModel;
      ...
      try {
        ...
        const URL2: string = multiMaskModelOnnx;
  ...
  const runModel = async () => {
    ...
    try {
      ...
      if (hasClicked) {
        ...
        const d = rleToImage(output.data, output.dims[0], output.dims[1]);
        setMaskImg(d);
